...
if exists (select 'x' from dbo.LOANS where KBCI_NO = @kbci_no and LOAN_STAT = 'R') begin
	...
	--while @@FETCH_STATUS = 0 begin
		...
		--		'
		--			update	dbo.S_ACCNT
		--			set		' + @loan_type + ' = ' + convert(nvarchar, ISNULL(@principal, 0)) + ' + ISNULL(' + @loan_type + ',0),
		--					PD' + @loan_type + ' = ' + convert(nvarchar, ISNULL(@pd, 0)) + '
		--			where	KBCI_NO = ''' + convert(nvarchar, @kbci_no) + '''
		--		';
	...
	--	exec sp_executesql @statement = @SQL;
		...
		--if @s_rema = 'DECEASED'
		--begin
		--	set @interest = case when @loan_type != 'STL' then 0 else @interest end
		--	set @penalty  = case when @loan_type != 'STL' then 0 else @penalty  end
		--	set @lri      = case when @loan_type != 'STL' then 0 else @lri      end
		--end
		
	--	set @totalInterest = @totalInterest + @interest;
	--	set @totalPenalty = @totalPenalty + @penalty;
	--	set @totalLri = @totalLri + @lri;
