...
/****** Object:  StoredProcedure sa.[dbo].sa.[Report_Maintenance_LoansStatement]    Script Date: 07/04/2009 22:50:13 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Report_Maintenance_LoansStatement]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Report_Maintenance_LoansStatement]
GO
...
CREATE PROCEDURE [dbo].[Report_Maintenance_LoansStatement]
@kbci_no varchar(7),
@my_user varchar(8)
AS
...
select
	...
	PD
from
	S_ACCNT_LOANS s with(nolock)
		inner join
	LOAN_TYPE lt with(nolock) on
		lt.LOAN_TYPE = s.LOAN_TYPE
where
	KBCI_NO = @kbci_no
order by
	LOAN_TYPE
