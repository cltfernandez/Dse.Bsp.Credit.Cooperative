...
/****** Object:  StoredProcedure [dbo].[Report_Admin_TotalInterestPaidByRange]    Script Date: 08/30/2014 14:40:26 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Report_Admin_TotalInterestPaidByRange]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Report_Admin_TotalInterestPaidByRange]
GO
...
CREATE PROCEDURE [dbo].[Report_Admin_TotalInterestPaidByRange]
@date_from date,
@date_to date
as
...
FROM
	...
	LEDGER LED ON
		LON.PN_NO = LED.PN_NO
WHERE
	LED.DATE BETWEEN @date_from AND @date_to AND
